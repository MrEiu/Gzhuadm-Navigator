    ...
    if (usePostgres) {
        try {
            const dbRes = await pgPool.query(
                'SELECT id, title, COALESCE(mode, \'admissions\') AS mode, messages, created_at AS "createdAt", updated_at AS "updatedAt" FROM chat_sessions WHERE username = $1 ORDER BY updated_at DESC',
                [username]
            );
    ...
    if (usePostgres) {
        try {
            for (const s of sessions) {
                const sessionMode = s.mode || (s.id && s.id.includes('group') ? 'group' : 'admissions');
                await pgPool.query(
                    `INSERT INTO chat_sessions (id, username, title, mode, messages, updated_at)
           VALUES ($1, $2, $3, $4, $5, $6)
           ON CONFLICT (id) DO UPDATE
           SET title = EXCLUDED.title, mode = EXCLUDED.mode, messages = EXCLUDED.messages, updated_at = EXCLUDED.updated_at`,
                    [s.id, username, s.title, sessionMode, JSON.stringify(s.messages), s.updatedAt || new Date().toISOString()]
                );
            }
        } catch (e) {
...
router.post('/profile', async (req, res) => {
    ...
    // Auto create / update initial background memory snippet in personal RAG for all students
    saveUserPersonalMemory(
        username,
        `学生个人基础背景资料：昵称【${updatedProfile.name || username}】，省份【${updatedProfile.province || '未填'}】，高考成绩【${updatedProfile.score ? `${updatedProfile.score}分` : '未填'}】，全省排名【${updatedProfile.rank ? `第${updatedProfile.rank}名` : '未填'}】，选科【${updatedProfile.subjects || '未填'}】，特殊情况说明【${updatedProfile.specialConditions || '无'}】`,
        '个人基础背景档案',
        '基础背景资料'
    ).catch(() => { });
